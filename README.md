...
- **JDK 25 or newer** installed and on PATH (or `JAVA_HOME` set). Check with `java -version`.
...
```
upi-offline-mesh/
├── pom.xml                                  Maven build, Spring Boot 3.3, Java 25
...
**`java: command not found`** — Install JDK 25+. On Windows, `winget install EclipseAdoptium.Temurin.25.JDK` or download from adoptium.net.
